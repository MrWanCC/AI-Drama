...
description: |
  ...
  钩子具象在全集前 3 拍内兑现、每场至少一个动作节拍、爽点认领、角色/场景/光照/道具对账上游……）。
...
| 钩子前 3 拍兑现 | 钩子不是标签——`hookBeat` 认领钩子具象落在[场,拍]，必须在全集前 3 拍内，否则冷开场空转 |
...
10 道质量门全是代码：每集时长在目标 ±15% 内（台词按 4.5 字/秒折算、动作按 2.5 秒/拍，`params` 可调）、单句台词 ≤ 35 字、说话人在本场人物里或标 `VO`、每集钩子悬念落纸且 `hookBeat` 认领的具象在全集前 3 拍内、每场至少一个动作节拍、动作叙述体不混引号台词、爽点认领、角色对账、场景/光照/道具对账。
...
报告含：KPI 带（含台词占比）、时长仪表（每集条形打在目标区间带上，超欠标红）、分集剧本（一排两集，场次信息超过 300px 渐隐截断、点开展开，钩子兑现的那一拍标红高亮）、场次总表、**台词本**（一排两个，按角色聚合、列表六行高可滚动、整组复制，给了 `--cast` 还带每角色音色提示词按钮，直接对接 TTS 批量生成）、质量门面板、导出 JSON（下载的就是 script.json 原样）。
...
129 项断言，不调模型、不花额度。10 道质量门每一道都有击穿用例（含 hook-open）。改完脚本先跑这个。
