...
python -X utf8 "${SCRIPTS_DIR}/fiction.py" --project-root "${PROJECT_ROOT}" init \
  --title "{书名}" --author "{作者}" --genre "{题材}"
```

`--project-root` 是全局参数，必须放在 `init` 子命令之前。初始化脚本只创建项目骨架；章节拆分、设定提取和 `writing_started` 标记仍由本 skill 的执行流程在用户确认后完成。
