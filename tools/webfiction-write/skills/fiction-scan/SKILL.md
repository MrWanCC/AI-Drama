...
description: |
  ...
  脚本采集优先，采集失败自动降级。browser-cdp 是外部可选依赖，本仓库不内置浏览器登录态能力。
...
优先运行对应平台脚本。起点使用移动端 SSR，默认不需要 Chrome 浏览器；番茄等需要浏览器态的平台再使用外部
`browser-cdp`。本仓库不内置该 skill；运行前必须确认宿主已经提供它或已有可用 CDP 端点。

如果 browser-cdp/Chrome 不可用，不要把失败的脚本输出当作实时数据：改用用户提供的榜单材料或内置知识，
并在报告头部标明 `数据来源：用户提供` 或 `数据来源：内置知识（非实时）`，同时写明采集日期。
...
| 番茄 | fanqie-rank-scraper.js | 外部 browser-cdp + Chrome（可选） |
| 晋江 | jjwxc-rank-scraper.js | 直接运行 |
| 七猫 | qimao-rank-scraper.js | 外部 browser-cdp + Chrome（可选） |
| 刺猬猫 | ciweimao-rank-scraper.js | 直接运行 |
| 点众短篇 | dz-browse-scraper.js | 外部 browser-cdp + Chrome（可选） |
| 黑岩 | heiyan-booklist-scraper.js | 外部 browser-cdp + Chrome（可选） |

**起点采集合集**（优先运行 
node scripts/qidian-rank-scraper.js，移动端 SSR 默认不需要 Chrome）：
...
**番茄采集合集**（需要外部 browser-cdp，通过详情页获取可读标题，绕过字体反爬）：

| 榜单 | URL 格式 | 核心字段 |
|------|---------|----------|
| 男频阅读榜 | fanqienovel.com/rank/1_2_{cat_id} | 在读数为核心指标 |
...
| scripts/fanqie-rank-scraper.js | 番茄榜单采集，需外部 browser-cdp |
| scripts/jjwxc-rank-scraper.js | 晋江榜单采集 |
| scripts/qimao-rank-scraper.js | 七猫榜单采集，需外部 browser-cdp |
| scripts/ciweimao-rank-scraper.js | 刺猬猫榜单采集 |
| scripts/cdp-utils.js | CDP 连接工具 |

爬虫脚本可配合外部 browser-cdp 使用。未安装时由主流程降级为用户材料或内置知识分析，必须标注为非实时，
不能声称已经完成平台实时采集。
