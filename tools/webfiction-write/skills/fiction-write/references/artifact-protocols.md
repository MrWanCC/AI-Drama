...
- 设定集/关系.md
- 设定集/题材定位.md
- 大纲/第{卷号}卷/节拍表.md、时间线.md、详细大纲.md
- 追踪/伏笔.md、时间线.md、角色状态.md、上下文.md
- 拆文库/{书名}/拆解报告.md（写作只读）

**层级关系：**
- `大纲/总纲.md` = 全书鸟瞰与不可随意漂移的主线。
- `大纲/第{卷号}卷/详细大纲.md` = 单卷和章节蓝图；写作前必须能定位本章。
- `正文/` = 已发生事实；正文与章纲冲突时，先记录差异，再由用户裁决是否回填章纲。

## 真源与投影边界

1. `.story-system/MASTER_SETTING.json`：开书时锁定的全局基线。
2. `.story-system/commits/chapter_{N}.commit.json`：写后事实、正文路径和哈希的提交记录。
3. `正文/`：最终叙事事实；脚本不会把“审查通过”伪写进正文。
4. `追踪/` 与 `.novel/state.json`：面向日常查询的投影，必须从已确认事实增量更新。
5. `大纲/`：计划，不是写后事实；变更传播先出影响报告，跨章修改需用户确认。

任何层缺失或不同步时，报告实际缺口。不要把兼容读取、空报告或未调用 reviewer 说成已验证。
...
此文件由 `fiction-analyze` 拆解管道输出（快速预览报告或完整拆文报告）。write skill 的职责是**读取**，不创建。
...
**状态含义**：`未埋` = 已规划但尚未埋设；`已埋` = 已埋设且仍开放；`已回收` = 已关闭；`已过期` = 错过预计回收窗口，需要 `/fiction-review` 或显式修复。日更只处理本轮新增/推进/回收的伏笔，全量伏笔审计由 `/fiction-review` 或用户明确要求触发。
