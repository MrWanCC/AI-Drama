...
| 3 | `正文/第{X-1}章_*.md` | 前一章（衔接） | 第1章时加载 `设定集/世界观.md` 或项目实际世界观文件替代 |
| 4 | `正文/第{X+1}章_*.md` | 后一章（衔接） | 末章时检查 `大纲/细纲_第{X+1}章.md`（如有）确保连续性 |
| 5 | `追踪/伏笔.md` | 涉及的伏笔 | 跳过 |
| 6 | `设定集/{相关角色}.md` | 本章涉及角色 | 跳过 |

**相关角色判定**：从当前卷详细大纲或逐章细纲中提取角色名。如果细纲不存在，从待修改章节正文中搜索 `设定集/` 下的角色卡关键词。以项目实际文件名为准，不凭空创建角色档案。
...
**资料研究（按需）**：如果修改涉及需要验证的外部事实（历史年代、地理方位、职业细节等），由当前 agent 明确提出核查需求并记录来源；不能声称调用了仓库中不存在的 `story-researcher` agent。
