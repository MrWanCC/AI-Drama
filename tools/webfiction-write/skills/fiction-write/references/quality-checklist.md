...
- 正文为空、编码损坏、含替换字符或不可见控制字符；
- 含模型拒答、自我介绍、占位符或明显写作工程话语；
- 角色/时间线/世界规则与本章已确认事实直接冲突；
- 正文跳过细纲核心事件，或提前释放后续阶段明确禁止的信息；
- 章末停在总结、作者预告或流程报告，而不是故事内的具体变化。

### Advisory（必须记录，不得机械替换）

- “仿佛/不禁/一丝”等模板词密度偏高；
- 单段过长、连续碎句或对话声线趋同；
- 本章没有显性爽点，但章节定位是关系回收、低压生活或信息整理；
- `追踪/` 尚未同步，或 reviewer 未调用/返回不完整。

### 证据规则

1. `scripts/check_prose.py` 只能证明确定性检查结果，不能证明文学质量。
2. reviewer 未成功返回结构化结果时，报告必须写“未验证”，不能写“通过”。
3. 写回、commit、指标文件只证明产物落盘，不等于审查通过。
4. 去 AI 味只在不改变事实、关系、时间线和情绪目标的范围内进行；需要新增剧情时转回 `fiction-write`。
