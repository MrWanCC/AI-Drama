...
```bash
python -X utf8 "${SCRIPTS_DIR}/fiction.py" --project-root "${PROJECT_ROOT}" project-memory add-pattern \
  --pattern-type "{归类}" --description "{完整描述}" --importance "{high|medium|low}"
```
...
- 输出包含 `pattern saved` 或 `pattern already exists, skipped`
