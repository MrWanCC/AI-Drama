...
按需读取 `.novel/state.json` 和 `大纲/总纲.md`；不要依赖 shell 的 `cat` 或聊天记忆替代项目文件。题材从 `state.json.project.genre` 读取，旧项目再兼容 `project_info`。
...
- 危机链至少 3 次递进
- 卷末新钩子
- 章节定位：推进/高压/训练/关系/呼吸/信息（可多选，但说明主次）
```
...
每章同时写明目标字数和节点预算；预算总和应落在 `[目标字数, 目标字数×1.1]`。低压、关系或信息章可以没有显性爽点/反转，但必须保留读者继续阅读的具体理由。
...
- 默认每 10 章一批
- 写完一批后只在完成报告中提示下一批细纲缺口，不自动开始生成或写正文
- 用户也可主动：(fiction-plan 11-20)

### Step 7：同步底本

每批章纲生成后，如项目已初始化 Story System，再同步对应底本；未初始化时报告缺口，不伪造同步成功：
```bash
