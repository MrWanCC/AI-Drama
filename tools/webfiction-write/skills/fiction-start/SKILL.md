...
python -X utf8 "${SCRIPTS_DIR}/fiction.py" --project-root "${PROJECT_ROOT}" init-contract
```
...
| 修改设定 | 随意改 | 改完需传播检测 |
| 写正文 | 不支持 | 正常 |
| 底本 | 不存在 | 执行 `init-contract` 后存在（版本号以输出为准）|
| fiction-write | 提示"请先 fiction-start" | 正常 |
| fiction-plan | 只做规划，不同步底本 | 规划 + 同步底本 |

**开书后每次 fiction-plan 只有在实际写入底本时才刷新对应版本号；若底本不存在或同步失败，必须报告未同步。**
