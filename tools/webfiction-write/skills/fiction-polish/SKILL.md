...
润色只改变表达、段落和标点，不新增剧情、不改变事实、人物关系、时间线或章纲目标。首次覆盖前保留可恢复副本；用户手工编辑过的正文必须先确认处理范围。
...
python -X utf8 "${SCRIPTS_DIR}/check_prose.py" --check "正文/第{章号}章-{标题}.md"
```

blocking findings 必须修复；advisory findings 只记录并结合全文判断，不做机械替换。检查通过也不等于独立 reviewer 已审查。
...
在用户允许的范围内写回目标正文，并保留原稿副本或版本记录；不自动级联修改后续章节，不覆盖用户手动编辑的内容。
...
如果使用外部标点脚本，必须先确认脚本实际存在；本仓库只保证 `scripts/check_prose.py` 和 `scripts/check_encoding.py` 两个确定性检查脚本。
