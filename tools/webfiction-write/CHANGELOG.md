...
### 修复
- `review-pipeline` 正确读取 `--review-results` 指向的 JSON 文件；文件缺失、损坏或 reviewer 标记 incomplete 时保持 `unverified`。
- `doctor` 在发现问题或找不到项目时返回非零退出码，JSON 输出可直接用于 CI。
- `init-contract` 变为幂等；只有底本内容发生变化时才递增版本号。
- `export` 去除常见 Markdown 展示标记，避免把标题、列表和链接语法带进纯文本。

### 新增
- `chapter-commit --data-artifacts` 记录 data-agent 已提供的三份 JSON artifact。
- `projections status|retry|replay --chapter N` 重建摘要投影并写入 `projection_log.jsonl`；未实现的宿主专属投影明确标记为 `skipped`。
- `fiction-review` 内置 review schema、通用 rubric、爽点/多线检查和阻断覆盖规则，并支持 reviewer 不可用时的 solo fallback。
- 扫榜文档明确 `browser-cdp` 为外部可选依赖，并要求标记非实时数据来源。

## [v1.2.0] - 2026-07-31

### 新增
- **正文确定性闸门**：新增 `scripts/check_prose.py`，检查编码、BOM、替换字符、控制字符、模型拒答/占位符、重复行，并将工程词、模板词、长段和总结式章尾列为 advisory。
- **写后证据协议**：新增日更停靠、细纲预算、最小状态写回和真源/投影边界参考，区分脚本检查、reviewer 审查、写回和 chapter commit。
- **可持久化审查指标**：`review-pipeline --save-metrics` 默认写入 `.novel/reviews/chapter_{N}.metrics.json`，并记录 `verified` 或 `unverified`。
- **章节提交事实**：`chapter-commit` 记录正文相对路径和 SHA-256 内容哈希，同时更新项目进度。

### 变更
- `init` 创建标准目录、`idea_bank.json`、`active-book` 和 schema v1 状态；已有状态不会被静默覆盖。
- `project-status` 支持 `--format json`；`doctor` 按开书状态检查总纲和追踪文件。
- 写作、审查、计划、导入、学习、导出和查询文档同步到实际脚本能力；跨章传播改为先报告、后确认。
- Windows/中文环境验证统一使用 UTF-8 模式；参考文件改为当前仓库本地路径和能力边界。

### 修复
- 修正 `fiction-import` 的 `--project-root` 参数顺序。
- 删除 `fiction-learn` 不支持的 `--category` 参数。
- 删除对不存在的 `index.db`、ZIP 归档、独立 story-* agent 和静默级联写回的错误声明。
- 清理 `fiction-scan` 中的控制字符，并让编码检查器报告替换字符和不可见控制字符。
- 修正 pre-commit 配置，递归检查整个仓库而不是只检查命中的第一个文件。
